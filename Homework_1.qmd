...
1. Collect and process raw data related to the outcome of interest
   i) Often completed in partnership with data engineers
2. Summarize and visualize the data to understand their general properties and tendencies
3. Model the data with an eye toward improved organizational decision-making or outcomes
4. Communicate findings in a way that makes the findings easily interpreted and actionable
...
Thus, we see that `y` has class "density." Its data type is "list," which is how R views the object internally. Finally, the output from the `str()` function indicates that `y` is a list with seven components. 
...
#Applying the plot function to y
plot(y)
```
